...
for cat in cats:
    ...
    for post in cat_posts:
        md = markdown.Markdown(extensions=['meta','footnotes','toc','codehilite','pymdownx.emoji','pymdownx.tilde','tables'], extension_configs={'codehilite': {'linenums': True}})
        # print(post)
        content = open(os.path.join(cat_path, post),encoding='utf-8-sig').read()
        ...
        if not '/{}/{}/{}'.format(cat_slug, post_year, post_month) in generate_list:
            generate_list.append('/{}/{}/{}'.format(cat_slug, post_year, post_month))
        if not '/{}/{}/{}/{}'.format(cat_slug, post_year, post_month, post_day) in generate_list:
            generate_list.append('/{}/{}/{}/{}'.format(cat_slug, post_year, post_month, post_day))
        if (len(content_meta["tags"]) == 1):
            content_meta['tags'] = content_meta['tags'][0].split(', ')
        for index in range(len(content_meta['tags'])):
            content_meta['tags'][index] = [content_meta['tags'][index], slugify(content_meta['tags'][index])]
        ...
        while content_meta['slug'] in slugs:
            rep_index += 1
            content_meta['slug'] = '{}-{}'.format(content_meta['slug'], rep_index)
        slugs.append(content_meta['slug'])
        if not '/{}/{}/{}/{}/{}'.format(cat_slug, post_year, post_month, post_day, content_meta['slug']) in generate_list:
            generate_list.append('/{}/{}/{}/{}/{}'.format(cat_slug, post_year, post_month, post_day, content_meta['slug']))
...
tags = collections.OrderedDict(sorted(tags.items(), key=lambda kv: -len(kv[1]['posts'])))
tags_list = {}
os.makedirs(os.path.join(output_directory,'tags'), exist_ok=True)
for tag, tag_things in tags.items():
    # dump to tags_list
    tags_list[tag_things['name']] = {'length': len(tag_things['posts']), 'slug': tag}
    # sort the posts
...
for single_post in posts_inside:
    json_name = os.path.join(output_directory, single_post['category'].lower(), '{}.json'.format(single_post['slug']))
